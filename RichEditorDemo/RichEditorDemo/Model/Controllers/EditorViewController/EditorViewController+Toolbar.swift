...
extension EditorViewController {
    func setupToolbar() {
        let toolbarView = UIView(frame: CGRect(x: 0, y: 0, width: bounds.width, height: 56))
...
//        } else {
            ...
            button.tag = action.rawValue
            button.translatesAutoresizingMaskIntoConstraints = false
            button.setImage(UIImage(named: "icn_back"), for: .selected)
            NSLayoutConstraint.activate([
                button.heightAnchor.constraint(equalToConstant: 40),
                button.widthAnchor.constraint(equalToConstant: 40)
            ])
...
////        colorPicker.popoverPresentationController?.sourceItem = toolbarButtons[action.rawValue]
        ...
        if toolbarCurrentColorPicker == .foregroundColor {
            editor.setForegroundColor(.random())
        } else if toolbarCurrentColorPicker == .backgroundColor {
            editor.setBackgroundColor(.random())
        }
