...
class PermissionManager: NSObject {
    ...
    func requestPermission(_ type: PermissionType, showScreen: Bool, completion: @escaping (Bool) -> Void) {
        DispatchQueue.main.async {
            
            switch type {
            case .location:
                if showScreen && !UserDefaults.locationShown{
                    UserDefaults.locationShown = true
                    self.openAllowPermissionScreen(type: .location)
                }else{
                    self.requestLocationPermission(completion)
                }
            case .contacts:
                if showScreen && !UserDefaults.contactsShown {
                    UserDefaults.contactsShown = true
                    self.openAllowPermissionScreen(type: .contacts)
                }else{
                    self.requestContactsPermission(completion)
                }
            case .microphone:
                if showScreen && !UserDefaults.microphoneShown {
                    UserDefaults.microphoneShown = true
                    self.openAllowPermissionScreen(type: .microphone)
                }else{
                    self.requestMicrophonePermission(completion)
                }
            case .speechRecognition:
                if showScreen && !UserDefaults.speechRecognitionShown {
                    UserDefaults.speechRecognitionShown = true
                    self.openAllowPermissionScreen(type: .speechRecognition)
                }else{
                    self.requestSpeechRecognitionPermission(completion)
                }
            case .calendar:
                if showScreen && !UserDefaults.calendarShown {
                    UserDefaults.calendarShown = true
                    self.openAllowPermissionScreen(type: .calendar)
                }else{
                    self.requestCalendarPermission(completion)
                }
            case .reminders:
                if showScreen && !UserDefaults.remindersShown {
                    UserDefaults.remindersShown = true
                    self.openAllowPermissionScreen(type: .reminders)
                }else{
                    self.requestRemindersPermission(completion)
                }
            case .camera:
                if showScreen && !UserDefaults.cameraShown {
                    UserDefaults.cameraShown = true
                    self.openAllowPermissionScreen(type: .camera)
                }else{
                    self.requestCameraPermission(completion)
                }
            case .photos:
                if showScreen && !UserDefaults.photosShown {
                    UserDefaults.photosShown = true
                    self.openAllowPermissionScreen(type: .photos)
                }else{
                    self.requestPhotosPermission(completion)
                }
            case .appTracking:
                if showScreen && !UserDefaults.appTrackingShown {
                    UserDefaults.appTrackingShown = true
                    self.openAllowPermissionScreen(type: .appTracking)
                }else{
                    self.requestAppTrackingPermission(completion)
                }
            case .localNetwork:
                if showScreen && !UserDefaults.localNetworkShown {
                    UserDefaults.localNetworkShown = true
                    self.openAllowPermissionScreen(type: .localNetwork)
                }else{
...
extension PermissionManager{
    func openAllowPermissionScreen(type:PermissionType){
        let permissionVC = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "PermissionDesignVC") as! PermissionDesignVC
        permissionVC.permissionType = type
        permissionVC.modalPresentationStyle = .fullScreen
        presentFromTopmostViewController(viewControllerToPresent: permissionVC, animated: true)
    }
...
extension UserDefaults {
    // Keys for storing the permission statuses
    private enum PermissionKeys: String {
        case locationShown
        case contactsShown
        ...
        case localNetworkShown
    }
    
    
    // Location Permission
    static var locationShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.locationShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.locationShown.rawValue)
        }
    }
    
    // Contacts Permission
    static var contactsShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.contactsShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.contactsShown.rawValue)
        }
    }
    
    // Microphone Permission
    static var microphoneShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.microphoneShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.microphoneShown.rawValue)
        }
    }
    
    // Speech Recognition Permission
    static var speechRecognitionShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.speechRecognitionShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.speechRecognitionShown.rawValue)
        }
    }
    
    // Calendar Permission
    static var calendarShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.calendarShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.calendarShown.rawValue)
        }
    }
    
    // Reminders Permission
    static var remindersShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.remindersShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.remindersShown.rawValue)
        }
    }
    
    // Camera Permission
    static var cameraShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.cameraShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.cameraShown.rawValue)
        }
    }
    
    // Photos Permission
    static var photosShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.photosShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.photosShown.rawValue)
        }
    }
    
    // App Tracking Permission
    static var appTrackingShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.appTrackingShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.appTrackingShown.rawValue)
        }
    }
    
    // App Tracking Permission
    static var localNetworkShown: Bool {
        get {
            return UserDefaults.standard.bool(forKey: PermissionKeys.localNetworkShown.rawValue)
        }
        set {
            UserDefaults.standard.set(newValue, forKey: PermissionKeys.localNetworkShown.rawValue)
        }
    }
}
