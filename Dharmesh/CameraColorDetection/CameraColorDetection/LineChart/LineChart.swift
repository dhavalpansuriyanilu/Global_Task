...
open class LineChart: UIView {
    ...
    */
    ...
    public struct Grid {
        ...
        // #eeeeee
        public var color: UIColor = UIColor(red: 238/255.0, green: 238/255.0, blue: 238/255.0, alpha: 0.2)
    }
    
    public struct Axis {
        public var visible: Bool = true
        // #607d8b
        public var color: UIColor = UIColor(red: 96/255.0, green: 125/255.0, blue: 139/255.0, alpha: 0.2)
    ...
    public struct Dots {
        ...
        public var innerRadius: CGFloat = 5
        public var outerRadius: CGFloat = 10
        public var innerRadiusHighlighted: CGFloat = 5
    ...
    open var lineColor: UIColor = UIColor(red: 234/255, green: 12/255, blue: 217/255, alpha: 1)
    // data stores
    ...
    fileprivate func drawDataDots(_ lineIndex: Int) {
        ...
        for index in 0..<data.count {
            ...
            // Get dot color from line color
            let dotColor = colors[lineIndex]
            
            // draw custom layer with another layer in the center
            let dotLayer = DotCALayer()
            dotLayer.dotInnerColor = self.lineColor //dotColor
            dotLayer.backgroundColor = self.lineColor.cgColor //dotColor.cgColor
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        // Change the line color based on your desired color
//        let lineColor: UIColor
        if lineIndex == 0 {
            self.lineColor = UIColor(red: 234/255, green: 12/255, blue: 217/255, alpha: 1) // EA0CD9
        } else {
            self.lineColor = UIColor(red: 28/255, green: 15/255, blue: 229/255, alpha: 1)   // 1C0FE5
        }
    ...
    fileprivate func drawAreaBeneathLineChart(_ lineIndex: Int) {
        ...
        // Create a gradient layer for the current line
        let gradient = CAGradientLayer()
        gradient.frame = self.bounds
        
        // Configure the gradient with lineColor at the top and black at the bottom
        gradient.colors = [self.lineColor.withAlphaComponent(0.6).cgColor, UIColor.black.withAlphaComponent(0.6).cgColor]
        gradient.locations = [0.0, 1.0]
        
        // Create a shape layer that will mask the gradient layer
        let shapeMask = CAShapeLayer()
        shapeMask.path = path.cgPath
        gradient.mask = shapeMask
        
        // Assign a unique name to the gradient layer for identification (optional)
        gradient.name = "gradientLayer_\(lineIndex)"
        
        // Remove previous gradient layer for this line if it exists
        if let oldGradientLayer = self.layer.sublayers?.first(where: { $0.name == "gradientLayer_\(lineIndex)" }) {
